...
class Agent:
    def __init__(self, action_space: gym.spaces.Discrete, observation_space: gym.spaces.Box, learning_rate=0.01, discount_factor=0.99):
    ...
    def learn(self, observation: gym.spaces.Box, action: gym.spaces.Discrete, reward: float, next_observation: gym.spaces.Box, done: bool) -> None:
        # Compute the TD error
        td_error = reward + self.discount_factor * np.max(np.dot(next_observation, self.weights)) - np.dot(observation, self.weights)[action]
